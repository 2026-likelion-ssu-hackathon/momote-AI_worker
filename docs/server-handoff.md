...
| `KAKAPO_PERSIST` | 선택 | (기억 저장소 파킹 2026-09-13 뒤로 효과 없음) |
...
### 2. 요청에 `speakerProfiles` (화자별 말투 기준선) — ✅ 합의 · ⏸️ 보류 (2026-09-13)
...
시드는 특정 커플 한 쌍의 값이라 실제로는 **모든 커플이 같은 기준선으로 판정**됩니다.

> ⏸️ **2026-09-13 — 보류.** 방을 사용자마다 만들 수 있게 되면서 시드 한 벌이 모든 방에 적용되는
> 문제가 생겼는데, 하루 시연이라 **워커가 공통 기준선 하나**(평균적인 카톡 말투)를 쓰는 것으로
> 정리했습니다. 서버 작업은 없습니다. 서비스를 이어가 방별 기준선이 필요해지면 아래 규격대로
> 실어주시면 됩니다 — 워커 쪽은 받는 자리가 있어 코드 변경 없이 바로 씁니다.
>
> **어디에** — 공통 분석 요청(`POST /internal/v1/chat-analyses`) 본문 최상위, `recentResults` 옆.
> 참여자 두 명 각각 1건. 없거나 한 명만 있어도 워커는 안 깨집니다 (그 화자만 폴백).
>
> **무엇으로** — 그 방에서 그 참여자가 보낸 **저장된 메시지 전체**(요청 창 30개가 아니라)로 집계.
> 표본이 넓을수록 "평소 대비"가 정확해집니다. 최근 N개로 자르신다면 300개 이상을 권합니다.
>
> | 필드 | 타입 | 산식 (워커 `worker/profile.py` 와 동일) |
> | --- | --- | --- |
> | `avgLength` | float | 메시지당 **공백을 뺀** 글자 수의 평균 |
> | `periodRate` | float 0~1 | 마침표 **하나로 끝나는** 메시지 비율. 정규식 `[^.]\.\s*$` — "..."·"…" 는 제외 |
> | `laughPerMsg` | float | 메시지당 `ㅋ`·`ㅎ` 글자 개수 평균 (한 메시지에 여러 개면 전부 셈) |
> | `emojiRate` | float 0~1 | 이모지가 **하나라도** 든 메시지 비율. 범위 `U+1F000–U+1FAFF`, `U+2600–U+27BF` |
> | `topAddress` | string[] ≤2 | 아래 호칭 목록 중 등장 횟수 상위 2개. 없으면 `[]` |
>
> 호칭 판정: 후보는 `오빠 언니 누나 형 자기야 자기 여보 애기야 애기 야 너 니가 네가 당신 그쪽` 15개.
> 메시지를 한글 어절로 나눠 **어절이 호칭으로 시작하고 나머지가 조사**
> (`는 가 도 의 한테 랑 이랑 와 과 을 를 만 은` 또는 없음)일 때만 셉니다 — "너무"의 '너',
> "거야"의 '야'가 잡히지 않게 하기 위해서입니다. 긴 후보부터 대조해 '자기야'가 '자기'로
> 잘리지 않게 합니다.
>
> **갱신 주기** — 하루 1회 배치면 충분합니다. 메시지마다 다시 계산할 필요 없습니다.
> 메시지가 0건인 참여자는 항목을 빼 주세요 (값을 0으로 채우지 마세요 — 워커가 "평소 길이 0"
> 으로 오해합니다).
>
> **워커 쪽 변경** — 없습니다. 필드가 오면 시드보다 우선 적용됩니다. 시드는 필드가 안 오는
> 방의 폴백으로만 남기고, 확인되면 시연 방 외에는 빼겠습니다.
